  ...
  const HorizonCloudCurtain: React.FC<{ count: number; radius: number; opacity: number }> = ({ count, radius, opacity }) => {
    const billboards = useMemo(() => {
      ...
      return Array.from({ length: clampedCount }, (_, i) => {
        ...
        const verticalAnchor = 20 + Math.random() * 50;
        const scale = 80 + Math.random() * 140;
        const planeHeight = scale * 0.6;
        // Lift the billboard just enough so its base sits near ground level.
        const y = Math.max(10, verticalAnchor + planeHeight * 0.45);
